 ...
 */
const getABVariantTitle = ( title, id ) => title || sprintf( __( 'Variant %s', 'altis' ), getLetter( id ) );
...
const ABTest = ( {
	...
	const test = block.ab_tests?.block;
		...
		<>
					...
					cards={ [
						{
							...
							description: __( 'Total number of times this block has been viewed.', 'altis' ),
						},
						{
							color: 'green',
							icon: 'thumbs-up',
							...
							description: aggregateData.hits && aggregateData.hits === 0
								...
								: __( 'Average conversion rate of the block as a percentage of total views of the block.', 'altis' ),
						},
						{
							color: 'blue',
							icon: 'chart-line',
							...
							description: __( 'The aggregated lift of all variants versus the original.', 'altis' ),
						},
					] }
				/>
			</div>
			...
			{ hasEnded && (
				<div className="altis-analytics-conclusion">
					<h2>{ __( 'Conclusion', 'altis' ) }</h2>
					{ winningVariantId === 0 && (
						<>
							<p>{ sprintf(
								__( '%s is the winner!', 'altis' ),
					...
					{ winningVariantId === false && (
						<>
							<p>{ __( 'No clear winner was found.', 'altis' ) }</p>
							<p>{ __( 'There were no statistically significant differences in the conversion rate between the variants.', 'altis' ) }</p>
						</>
					) }
				</div>
			) }
			...
			<Variants
				...
				append={ ( { variant } ) => {
					...
					return (
						<li>
							<p className="description">{ __( 'Probability of best', 'altis' ) }</p>

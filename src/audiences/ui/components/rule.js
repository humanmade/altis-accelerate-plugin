		...
		case 'number':
			return (
				<input
					...
					name={ name }
					placeholder={ `${ sprintf(
						__( 'Avg: %d, Lowest: %d, Highest: %d', 'altis' ),
					...
					return (
						<Fragment>
							...
							{ showDropdown && (
								...
								>
									<option value="">{ __( 'Empty', 'altis' ) }</option>
									{ topHits.length > 0 && (
										<optgroup label={ __( 'Top hits', 'altis' ) }>
									...
									{ allowFreeText && (
										<option value="___">{ __( 'Other, please specify...', 'altis' ) }</option>
									) }
								</select>
							) }
						</Fragment>
		...
		<StyledRule className="audience-editor__rule">
			...
			>
				<option
					...
					value=""
				>
					{ __( 'Select a field', 'altis' ) }
				</option>
				<optgroup label={ __( 'Persistent Data', 'altis' ) }>
				...
				<optgroup label={ __( 'Point-in-time Data', 'altis' ) }>

...
export type UpdatablePost = Post | PostUpdateObject;
...
export type Duration = 'P7D' | 'P14D' | 'P30D' | 'P60D' | 'P90D' | 'P1M' | Period;
...
export interface RefreshPostsAction extends StandardAction {
	type: 'REFRESH_POSTS',
}
...
export interface RemovePostAction extends StandardAction {
	type: 'REMOVE_POST',
	id: number,
}
...
export type Action = SetPostsAction | SetPostAction | SetStatsAction | SetIsLoadingAction
					| SetIsLoadingStatsAction | SetPaginationAction | RemovePostAction | RefreshPostsAction
 ...
 *
 * @param {number} value Number to pad with leading zero.
 * @returns
 */
...
/**
 * Converts from standard REST API to Top endpoint post shape.
 *
 * @param {object} post Standard REST API Post object.
 * @returns {Post} Modified post object.
 */
export function convertPostResponse( post: { [ k:string ] : any } ) : Post {
	const modified = {
		id: post.id,
	...
	if ( post?.title?.rendered ) {
		modified.title = post.title.rendered;
	}

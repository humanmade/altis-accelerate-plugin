...
const fetchBroadcasts = function ( keyword, extraArgs ) {
	const args = {
...
const fetchBroadcast = function ( id, extraArgs ) {
	const args = {
...
const EditComponent = function ( props ) {
	...
	// Create debounced versions for this component instance only.
	const debouncedFetchBroadcast = useCallback( debouncePromise( fetchBroadcast, 1000 ), [] ); // eslint-disable-line react-hooks/exhaustive-deps
	const debouncedFetchBroadcasts = useCallback( debouncePromise( fetchBroadcasts, 1000 ), [] ); // eslint-disable-line react-hooks/exhaustive-deps
	...
	useEffect( () => {
		...
		if ( attributes.broadcast && ! broadcast?.id ) {
			setFetchError( false );
			debouncedFetchBroadcast( attributes.broadcast, { signal: abortController.signal } )
				.then( setBroadcast )
				.catch( response => {
	...
	}, [ attributes.broadcast, setBroadcast, broadcast, debouncedFetchBroadcast ] );

	// Search broadcasts, and load initial broadcast list.
	useEffect( () => {
		const abortController = new AbortController();
		setIsFetching( true );
		debouncedFetchBroadcasts( searchKeyword, { signal: abortController.signal } )
			.then( setBroadcasts )
			.catch( () => {
				setBroadcasts( [] );
			} )
		...
		return () => abortController.abort();
	}, [ searchKeyword, debouncedFetchBroadcasts ] );
							...
							<div className="altis-broadcast-blocks-inserter__title">
								{
									fetchError
										...
										: broadcast
											? (
												<>
													<div className="altis-broadcast-blocks-inserter__current">
													...
													<div className="altis-broadcast-blocks-inserter__thumbnail-list">
														{ broadcast.thumbnails.slice( 0, 3 ).map( thumbnailUrl => (
															<div key={ thumbnailUrl } className="altis-broadcast-blocks-inserter__thumbnail">
																<Image
																	alt={ '' }
																	height={ 80 }
																	src={ thumbnailUrl }
										...
										: broadcasts.length > 0
											? (
												<ul className="altis-broadcast-block-list">
													{
														broadcasts.map( item => (
															<li key={ item.id } className="altis-broadcast-block-list__list-item">

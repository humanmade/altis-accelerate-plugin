...
class Posts_Controller extends WP_REST_Posts_Controller {
	...
	public function register_routes() {
		...
		register_rest_field( Blocks\POST_TYPE, 'subtype', [
			...
			'schema' => [
				'type' => 'string',
				'description' => __( 'The type of experience block', 'altis' ),
			],
		] );
	...
	public function get_subtype( array $post ) : string {
		return (string) ( get_post_meta( $post['id'], '_xb_type', true ) ?: 'standard' );
	}
	...
	public function get_variants( array $post ) : array {
		...
		$subtype = $this->get_subtype( $post );

		foreach ( $blocks as $id => $variant ) {
			if ( $variant['blockName'] !== 'altis/variant' ) {
				continue;
			}
			...
			switch ( $subtype ) {
				case 'personalization':
					if ( isset( $variant['attrs']['audience'] ) && intval( $variant['attrs']['audience'] ) !== 0 ) {
						...
						$variant['attrs']['id'] = 0;
						$variant['attrs']['title'] = __( 'Fallback', 'altis' );
						// Always add the fallback to the start of the array.
						array_unshift( $variants, $variant['attrs'] );
					}
					break;
				default:
					$default_title = sprintf( __( 'Variant %s', 'altis' ), Utils\get_letter( $id ) );
					$variant['attrs']['id'] = $id;
	...
	protected function prepare_links( $post ) {
		...
		$links['wp:analytics'] = [
			'href' => rest_url( $this->namespace . '/' . $this->rest_base . '/' . $post->ID . '/views' ),
			'embeddable' => true,
		];

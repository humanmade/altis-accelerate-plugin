...
function ToolbarButton() {
	...
	const onSubmitPrompt = async ( prompt ) => {
		...
		if ( selectedBlocks.length === 1 && selectedBlocks[0]?.name === 'altis/ai' ) {
			const newAttributes = {
			...
			const wrapped = switchToBlockType( selectedBlocks, 'altis/ai' );
...
export default function ToolbarButtonWrap( BlockEdit ) {
	return props => {
		...
		// Get the parent blocks using the getBlock function. We don't want to show the toolbar button if the parent is an altis/ai block.
		...
		const isAiBlockWithNoInnerBlocks = props.name === 'altis/ai' && innerBlocks.length === 0;
		if ( props.name === 'core/image' || blockParents.filter( block => block.name === 'altis/ai' ).length > 0 || isAiBlockWithNoInnerBlocks ) {
			return (
				<BlockEdit
					key="edit"
					{ ...props }

...
namespace Altis\Accelerate\GlobalBlocks\REST_API\Search;
	...
	public function get_items( $request ) {
		...
		if ( empty( $search_id ) ) {
			return new WP_Error(
				...
				__( 'No `searchID` parameter provided.', 'altis' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( ! is_numeric( $search_id ) ) {
			return new WP_Error(
				...
				__( 'Invalid `searchID` parameter provided.', 'altis' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( ! $post = get_post( intval( $search_id ) ) ) {
			...
			return new WP_Error(
				...
				sprintf( __( 'The requested post ID of %d not found.', 'altis' ), $search_id ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( $response->is_error() ) {
			return new WP_Error(
				...
				__( 'There was an error encountered when retrieving blocks from ID.', 'altis' ),
				[ 'status' => 404 ]
			);
		}

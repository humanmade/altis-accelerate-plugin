...
namespace Altis\Accelerate\GlobalBlocks\REST_API\Relationships;
...
class REST_Endpoint {
	...
	public function __construct() {
		$this->namespace = 'altis-global-blocks/v1';
			...
			[
				[
					...
					'args'                => [
						...
						'block_id' => [
							'description' => esc_html__( 'Block ID to get the relationship data for.', 'altis' ),
							'required'    => true,
							'type'        => 'integer',
						],
					],
	...
	public function get_item_schema() : array {
		$schema = [
			...
			'title'      => __( 'Block relationships', 'altis' ),
			'type'       => 'object',
			'properties' => [
				'id'     => [
	...
	public function get_items( $request ) {
		...
		if ( empty( $block_id ) ) {
			return new WP_Error(
				...
				__( 'No `block_id` parameter provided.', 'altis' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( ! $post = get_post( $block_id ) ) {
			...
			return new WP_Error(
				...
				sprintf( __( 'The requested post ID of %d not found.', 'altis' ), $block_id ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( $post->post_type !== 'wp_block' ) {
			return new WP_Error(
				...
				__( 'The requested post ID was not of the post type `wp_block`.', 'altis' ),
				[ 'status' => 404 ]
			);
		}
		...
		$query_args = [
			'posts_per_page' => GlobalBlocks\RELATIONSHIPS_PER_PAGE,
			'paged'          => $page ?? 1,
		...
		$max_pages = ceil( $total_posts / GlobalBlocks\RELATIONSHIPS_PER_PAGE );

		// Return error if requested invalid page number.
		if ( $page > $max_pages && $total_posts > 0 ) {
			return new WP_Error(
				'rest_post_invalid_page_number',
				__( 'The page number requested is larger than the number of pages available.', 'altis' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $response->is_error() ) {
			return new WP_Error(
				...
				__( 'Encountered error retrieving relationship data.', 'altis' ),
				[ 'status' => 404 ]
			);
		}

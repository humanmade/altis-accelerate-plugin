...
namespace Altis\Accelerate\Audiences\REST_API;
...
function init() {
	...
	register_rest_route( 'accelerate/v1', 'audiences/fields', [
		[
			...
			'callback' => 'Altis\\Accelerate\\Audiences\\get_field_data',
			'permission_callback' => __NAMESPACE__ . '\\check_edit_permission',
		],
		'schema' => function () {
			return [
				'$schema' => 'http://json-schema.org/draft-04/schema#',
				'title' => 'Get field data for creating audiences',
				'type' => 'array',
				'items' => [
					'type' => 'object',
					'properties' => [
	...
	register_rest_route( 'accelerate/v1', 'audiences/estimate', [
		[
			...
			'args' => [
				'audience' => [
					'description' => __( 'A URL encoded audience configuration JSON string', 'altis' ),
					'required' => true,
					'type' => 'string',
	...
	if ( json_last_error() !== JSON_ERROR_NONE ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Could not decode JSON: %s', 'altis' ),
				json_last_error_msg()
			)
		);
	}

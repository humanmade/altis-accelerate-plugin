...
namespace Altis\Tests\Unit\GlobalBlocks;

use Altis\Tests\Unit\TestCase;
use Altis\GlobalBlocks as Testee;
use const Altis\GlobalBlocks\Connections\POST_POST_TYPE;
...
class NamespaceTest extends TestCase {
	...
	public function test_bootstrap() {
		...
		Actions\expectAdded( 'enqueue_block_editor_assets' )
			->with( 'Altis\GlobalBlocks\enqueue_block_editor_assets' );

		Actions\expectAdded( 'admin_bar_menu' )
			->with( 'Altis\GlobalBlocks\add_block_admin_bar_menu_items', 100 );

		Actions\expectAdded( 'admin_menu' )
			->with( 'Altis\GlobalBlocks\admin_menu', 9 );

		Filters\expectAdded( 'wp_insert_post_data' )
			->with( 'Altis\GlobalBlocks\insert_reusable_block_post_data', 10, 2 );

		Filters\expectAdded( 'register_post_type_args' )
			->with( 'Altis\GlobalBlocks\show_wp_block_in_menu', 10, 2 );

		Filters\expectAdded( 'allowed_block_types_all' )
			->with( 'Altis\GlobalBlocks\filter_allowed_block_types', 20 );

		Filters\expectAdded( 'block_categories_all' )
			->with( 'Altis\GlobalBlocks\rename_block_category' );

		Actions\expectAdded( 'registered_post_type_wp_block' )
			->with( 'Altis\GlobalBlocks\update_reusable_block_registration', 10, 2 );

		Actions\expectAdded( 'enqueue_block_editor_assets' )
			->with( 'Altis\GlobalBlocks\override_translation_strings', 11 );

		Filters\expectAdded( 'block_type_metadata' )
			->with( 'Altis\GlobalBlocks\override_block_type_metada' );
	...
	public function test_filter_allowed_block_types_array() {
		$this->assertSame( Testee\filter_allowed_block_types( [] ), [ 'altis/global-block' ] );
	}
